...
-- =========================================================
-- Correção de acesso para o site publicado no GitHub Pages
-- Rode este arquivo inteiro no SQL Editor do Supabase.
-- Ele garante permissões, RLS, perfil Coordenação e confirma as contagens.
-- =========================================================

grant usage on schema public to anon, authenticated;

grant select on table public.limpeza to authenticated;
grant insert, update, delete on table public.limpeza to authenticated;
...
grant execute on function public.is_coordenacao() to authenticated;
grant execute on function public.can_write_pdm() to authenticated;

-- Garante que o usuário principal tenha perfil Coordenação.
-- Se o seu e-mail for outro, troque abaixo e rode novamente.
insert into public.profiles (user_id, nome, email, role)
select id, coalesce(raw_user_meta_data->>'nome', email), email, 'coordenacao'
from auth.users
where email = 'erwinklein1994@gmail.com'
...
-- Diagnóstico final: precisa retornar limpeza=133 e obras=15.
...
-- Diagnóstico do usuário principal: precisa retornar role=coordenacao.
select u.email, p.role
from auth.users u
left join public.profiles p on p.user_id = u.id
where u.email = 'erwinklein1994@gmail.com';
